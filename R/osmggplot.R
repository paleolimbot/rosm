...
StatOSM <- ggplot2::ggproto("StatOSM", ggplot2::Stat,
   ...
   compute_group = function(data, scales, obj=NULL, zoomin=0, zoom=NULL,
     ...
     fused <- osm.raster(x=obj, zoomin=zoomin, zoom=zoom, type=type, forcedownload=forcedownload,
                         cachedir=cachedir, projection=projection, .rawfused=TRUE)
     bbox <- .projectbbox(.revprojectbbox(fused[[2]], fromepsg=3857), projection=projection)
     nbrow <- dim(fused[[1]])[1]
     nbcol <- dim(fused[[1]])[2]
     bands <- dim(fused[[1]])[3]
     # melt for xyz data frame form
     fused <- reshape2::melt(fused[[1]], varnames=c("y", "x", "z"))
     # scale columns to 0 - 255 range
     fused$value <- fused$value * 255
     # cast for easier paste0-ing
     fused <- reshape2::dcast(fused, x + y ~ z)
     # transform coordinates from pixels to final coords from bbox
     fused$x <- bbox[1,1]+(fused$x-1)/nbcol*(bbox[1,2]-bbox[1,1])
     fused$y <- bbox[2,1]+(fused$y-nbrow)/nbrow*(bbox[2,1]-bbox[2,2])
     # make colours from rgb values
     fused$fill <- paste0("#", do.call(paste0, lapply(fused[c("1", "2", "3")], as.hexmode)))
