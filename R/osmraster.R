...
osm.raster <- function(x, zoomin=0, zoom=NULL, type="osm", forcedownload=FALSE, cachedir=NULL,
  ...
  if(methods::is(x, "Spatial")) {
    ...
    if(!is.na(rgdal::CRSargs(x@proj4string))) {
      ...
      lookup.bbox <- sp::bbox(sp::spTransform(x, CRS("+init=epsg:4326")))
    }
...
#' Plot an OSM RasterStack as generated by \link{osm.raster}.
#' This function is a thin wrapper to \code{raster::plotRGB()}.
#'
#' @param rgbraster An RGB RasterStack as generated by \link{osm.raster}
#' @param ... Arguments to be passed on to \code{raster::plotRGB()}.
#' @export
osm.raster.plot <- function(rgbraster, ...){
